...
const Sidebar = ({ isAuthenticated, setIsAuthenticated, fetchNotes }) => {
  ...
  return (
    <>
      <button
        className="md:hidden p-2 m-2 bg-blue-600 text-white rounded-lg"
      ...
      <div
        className={`fixed top-0 left-0 h-screen bg-blue-900 p-4 w-60 flex flex-col transition-transform duration-300 ${isOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0 overflow-y-auto z-50`}
      >
        <button
        ...
        <div className="px-4 text-xl text-white space-y-3">
          ...
          <h1 className="cursor-pointer hover:text-gray-300" onClick={handleRecordNote}>
            Add Notes
          </h1>
        </div>
        ...
        <div className="mt-auto flex flex-col gap-2 pb-4">
          {isAuthenticated ? (
            <button
              ...
              className="w-full p-3 text-xl text-white text-center rounded-lg shadow-xl cursor-pointer hover:bg-gray-700"
            >
              Logout
            </button>
          ) : (
